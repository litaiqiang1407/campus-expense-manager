...
const fetchTransactionData = async () => {
    try {
        ...
        note.value = router.query.note || transactionData.note;
        ...
        transactionDate.value = new Date(transactionData.date);
    } catch (error) {
...
const selectWallet = () => {
    routerr.push({ name: 'SelectWallet', query: { transactionId: transactionId } });
};
...
const submitForm = async () => {
    try {
        const formData = {
            ...
            amount: amount.value,
            wallet_id: router.query.walletId || wallet_id.value,
            note: note.value,
