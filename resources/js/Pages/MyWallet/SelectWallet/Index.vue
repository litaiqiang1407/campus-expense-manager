<template>
    <div class="max-w-full mx-auto mt-4">
        <div v-for="wallet in wallets" :key="wallet.id">
            <div v-if="wallet.name === 'Total'"
                class="bg-white rounded-lg flex items-center p-4 shadow-sm w-full h-full mb-2">
                <div class="w-full flex items-center justify-between" @click="selectWallet(wallet.id)">
                    <div class="flex flex-1 items-center">
                        <img :alt="`${wallet.name} icon`" class="w-10 h-10 rounded-full" :src="wallet.icon_path" />
        ...
        <div v-if="openWalletTypes" class="fixed inset-0 bg-black bg-opacity-50 flex items-end justify-center z-50"
            @click="closeWalletTypes">
            <div class="bg-white w-full rounded-t-lg p-4 z-100" @click.stop>
                <div class="w-full p-2 flex items-center justify-between">
                    ...
                    <div class="button-animate size-7 flex items-center justify-center rounded-full border-[1px] border-primary"
                        @click="closeWalletTypes">
                        <font-awesome-icon icon="xmark" class="text-[16px]" />
                    </div>
                </div>
                <div class="px-2 py-4 grid grid-cols-2 gap-4">
                    <button v-for="walletType in walletTypes" :key="walletType.id"
